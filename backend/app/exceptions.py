...
class TicketPilotException(Exception):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        # Derive a stable error code from the class name (e.g. "NotFoundError" → "NOT_FOUND")
        raw = self.__class__.__name__
        code = "".join(f"_{c}" if c.isupper() else c for c in raw).strip("_").upper()
        return {
...
class UnauthorizedError(TicketPilotException):
    ...
    def __init__(
        ...
        super().__init__(message=message, status_code=401, details=details or {})
...
class TicketNotFoundError(NotFoundError):
    ...
    def __init__(self, ticket_id: str):
        super().__init__(
            message=f"Ticket not found", resource_type="ticket", resource_id=ticket_id
        )
...
class BusinessLogicError(TicketPilotException):
    ...
    def __init__(
        self, message: str, rule: Optional[str] = None, details: Optional[Dict] = None
    ):
        details = details or {}
        if rule:
...
class OrganizationMembershipError(BusinessLogicError):
    ...
    def __init__(self, org_name: Optional[str] = None):
        ...
        super().__init__(message=message, rule="organization_membership_required")
...
class InvalidAssignmentError(BusinessLogicError):
    ...
    def __init__(self, reason: str):
        super().__init__(
            message=f"Cannot assign ticket: {reason}", rule="valid_ticket_assignment"
        )
...
def require_resource_ownership(
    resource_org_id: str, user_org_id: str, resource_type: str, resource_id: str
):
    """
