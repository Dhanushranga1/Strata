...
def validate_email(value: str) -> str:
    ...
    if not EMAIL_REGEX.match(value):
        raise ValidationError(
            "Invalid email format. Please enter a valid email address.", field="email"
        )
...
def validate_slug(value: str, field_name: str = "slug") -> str:
    ...
    if len(value) < 3:
        raise ValidationError(
            f"{field_name} must be at least 3 characters long", field=field_name
        )

    if len(value) > 50:
        raise ValidationError(
            f"{field_name} must be at most 50 characters long", field=field_name
        )
    ...
    if value in reserved:
        raise ValidationError(
            f"'{value}' is a reserved slug and cannot be used", field=field_name
        )
...
def validate_enum(
    value: str, allowed_values: List[str], field_name: str, case_sensitive: bool = False
) -> str:
    """
