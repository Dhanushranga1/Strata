...
const KBIngestModal = forwardRef<HTMLDivElement, KBIngestModalProps>(
  ({
    ...
    const addFileSource = useCallback((files: FileList | null) => {
      if (!files) return;
      
      const newSources: IngestSource[] = [];
      
      for (let i = 0; i < files.length && sources.length + newSources.length < maxSources; i++) {
        const file = files[i];
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent 
          ref={ref}
          className={cn("max-w-4xl max-h-[80vh] overflow-hidden flex flex-col", className)}
          {...props}
        >
          <DialogHeader>
              ...
              <div className="flex-1 overflow-hidden">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Badge variant="outline">
                    Total: {formatFileSize(sources.reduce((sum, s) => sum + (s.size || 0), 0))}
                  </Badge>
                </div>
