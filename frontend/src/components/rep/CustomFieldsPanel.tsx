...
export function CustomFieldsPanel({ ticketId, orgId }: CustomFieldsPanelProps) {
  ...
  const [values, setValues] = useState<Record<string, { text?: string; number?: number; date?: string; bool?: boolean }>>({})
  const debounceRef = useRef<ReturnType<typeof setTimeout> | null>(null)

  useEffect(() => {
    api.get<TicketFieldEntry[]>(`/api/custom-fields/tickets/${ticketId}`, orgId)
      .then(data => {
        setEntries(data)
  ...
  const save = useCallback((updated: typeof values) => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      api.patch(`/api/custom-fields/tickets/${ticketId}`, { values: payload }, orgId).catch(() => {})
    }, 500)
            ...
            <div key={f.id} className="space-y-1">
              ...
              {f.field_type === 'boolean' ? (
                ...
                <Input
                  ...
                  value={val.number ?? ''}
                  onChange={e => update(f.id, { number: e.target.value ? parseFloat(e.target.value) : undefined })}
                />
              ) : f.field_type === 'date' ? (
                <Input
                  type="date"
                  className="h-8 text-xs"
                  value={val.date ?? ''}
                  onChange={e => update(f.id, { date: e.target.value || undefined })}
                />
              ) : (
                <Input
                  type="text"
                  className="h-8 text-xs"
                  value={val.text ?? ''}
                  onChange={e => update(f.id, { text: e.target.value || undefined })}
                />
              )}
            </div>
          )
