...
import { buildAISuggestionQuery, prepareTicketContext, redactPII } from '@/lib/ai/prompt'
  ...
  describe('prepareTicketContext', () => {
    ...
    it('should handle unknown sender roles', () => {
      const messagesWithUnknown = [
        { sender_role: 'unknown', body: 'Test message', timestamp: '2025-01-01T10:00:00Z' }
      ]
  ...
  describe('buildAISuggestionQuery', () => {
    ...
    it('should limit to 5 recent messages', () => {
      const manyMessages = Array.from({ length: 10 }, (_, i) => ({
        role: 'customer' as const,
        text: `Message ${i + 1}`,
        timestamp: new Date(`2025-01-01T10:${i.toString().padStart(2, '0')}:00Z`)
      }))
