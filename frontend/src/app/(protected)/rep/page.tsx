...
export default function RepConsolePage() {
  ...
  // SWR — serves cache on tab-switch, revalidates in background, auto-refreshes every 30s
  const queueKey = (user && isReady && orgId)
    ? ['rep-queue', orgId, currentLane, String(offset), searchQuery, String(mineOnly)]
    : null
  const countsKey = (user && isReady && orgId) ? ['rep-counts', orgId] : null

  const { data: queueData, isLoading: ticketsLoading, mutate: mutateQueue } = useSWR(
    queueKey,
    async ([, org, lane, off, q, mine]: string[]) => {
      const params = new URLSearchParams({ lane, offset: off, limit: String(limit) })
      if (q) params.append('q', q)
      if (mine === 'true') params.append('mine', 'true')
      return api.get(`/api/rep/queue?${params}`, org)
    },
    { refreshInterval: 30000, onSuccess: () => setLastRefresh(Date.now()) }
  )
  const { data: countsData, mutate: mutateCounts } = useSWR(
    countsKey,
    async ([, org]: string[]) => api.get(`/api/rep/counts`, org),
    { refreshInterval: 30000 }
  )
  const tickets: QueueItem[] = queueData?.items ?? []
  const counts: QueueCounts = countsData ?? { needs_attention: 0, open_active: 0, in_progress: 0, escalated: 0, all: 0, resolved_today: 0 }
  ...
  const handleManualRefresh = async () => {
    ...
    try {
      await Promise.all([mutateQueue(), mutateCounts()])
