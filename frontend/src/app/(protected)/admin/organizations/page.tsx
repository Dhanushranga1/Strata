...
export default function AdminOrganizationsPage() {
  ...
  // Edit org dialog
  const [editDialog, setEditDialog] = useState<{ open: boolean; org: OrgRow | null }>({ open: false, org: null })
  const [editName, setEditName] = useState('')
  const [editLoading, setEditLoading] = useState(false)

  // Add member dialog
  const [addMemberDialog, setAddMemberDialog] = useState<{ open: boolean; orgId: string | null }>({ open: false, orgId: null })
  const [addEmail, setAddEmail] = useState('')
  const [addRole, setAddRole] = useState('rep')
  const [addLoading, setAddLoading] = useState(false)

  // Remove member confirmation
  const [removeDialog, setRemoveDialog] = useState<{ open: boolean; orgId: string; member: OrgMember | null }>({ open: false, orgId: '', member: null })
  ...
  const handleEdit = async () => {
    ...
    setEditLoading(true)
    try {
      await api.patch(`/api/admin/organizations/${editDialog.org.id}`, { name: editName.trim() })
  ...
  const toggleActive = async (org: OrgRow) => {
    try {
      await api.patch(`/api/admin/organizations/${org.id}`, { is_active: !org.is_active })
      toast.success(org.is_active ? 'Organisation deactivated' : 'Organisation reactivated')
      await loadOrgs()
    } catch (e: any) {
  ...
  const handleAddMember = async () => {
    ...
    setAddLoading(true)
    try {
      await api.post(`/api/admin/organizations/${addMemberDialog.orgId}/members`, {
        email: addEmail.trim(),
        role: addRole,
      })
      ...
      setMembersMap(prev => { const next = { ...prev }; delete next[orgId]; return next })
      loadMembers(orgId)
    } catch (e: any) {
  ...
  const handleRoleChange = async (orgId: string, userId: string, newRole: string) => {
    try {
      await api.patch(`/api/admin/organizations/${orgId}/members/${userId}`, { role: newRole })
      setMembersMap(prev => ({
        ...prev,
        [orgId]: (prev[orgId] || []).map(m => m.user_id === userId ? { ...m, role: newRole } : m),
      }))
  ...
  const handleRemove = async () => {
    ...
    try {
      await api.delete(`/api/admin/organizations/${removeDialog.orgId}/members/${removeDialog.member.user_id}`)
      toast.success(`Removed ${removeDialog.member.user_email ?? removeDialog.member.user_id}`)
      const orgId = removeDialog.orgId
      setMembersMap(prev => ({
        ...prev,
        [orgId]: (prev[orgId] || []).filter(m => m.user_id !== removeDialog.member!.user_id),
      }))
  ...
  return (
    <PageShell>
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-16 text-muted-foreground">
              ...
              <Button className="mt-4" onClick={() => setCreateDialog(true)}>Create first organisation</Button>
            </CardContent>
          </Card>
        ) : (
          <div className="space-y-3">
            {orgs.map(org => (
              <Card key={org.id} className={cn('border', !org.is_active && 'opacity-60')}>
                <CardContent className="py-4 px-5">
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    >
                      ...
                      {!org.is_active && <Badge variant="outline" className="text-xs border-orange-400 text-orange-600">Inactive</Badge>}
                    </button>
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground shrink-0">
                      ...
                      <span>{org.ticket_count} ticket{org.ticket_count !== 1 ? 's' : ''}</span>
                    </div>

                    <div className="flex items-center gap-2 shrink-0">
                      <Button size="sm" variant="ghost" onClick={() => openEdit(org)} title="Rename">
                        <Pencil className="h-3.5 w-3.5" />
                      </Button>
                      <Button size="sm" variant="ghost" onClick={() => toggleActive(org)}
                        title={org.is_active ? 'Deactivate' : 'Reactivate'}>
                        {org.is_active
                          ? <ToggleRight className="h-4 w-4 text-green-600" />
                          : <ToggleLeft className="h-4 w-4 text-muted-foreground" />}
                      </Button>
                      <Button size="sm" variant="outline" onClick={() => openAddMember(org.id)}>
                        ...
                        <div className="space-y-2">
                          {(membersMap[org.id] || []).map(m => (
                            <div key={m.user_id} className="flex items-center gap-3 text-sm">
                              ...
                              <span className="flex-1 truncate text-sm">{m.user_email ?? m.user_id}</span>
                              <span className="text-xs text-muted-foreground hidden sm:block">{timeAgo(m.last_sign_in_at)}</span>
                              <Select
                                value={m.role}
                                onValueChange={v => handleRoleChange(org.id, m.user_id, v)}
                              >
                                <SelectTrigger className={cn('h-7 w-32 text-xs px-2', roleBadgeClass[m.role])}>
                                  <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
      ...
      {/* Edit org dialog */}
      <Dialog open={editDialog.open} onOpenChange={open => !open && setEditDialog({ open: false, org: null })}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditDialog({ open: false, org: null })}>Cancel</Button>
      ...
      {/* Add member dialog */}
      <Dialog open={addMemberDialog.open} onOpenChange={open => !open && setAddMemberDialog({ open: false, orgId: null })}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setAddMemberDialog({ open: false, orgId: null })}>Cancel</Button>
      ...
      <Dialog open={removeDialog.open} onOpenChange={open => !open && setRemoveDialog({ open: false, orgId: '', member: null })}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Remove Member</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground py-2">
            Remove <strong>{removeDialog.member?.user_email ?? 'this member'}</strong> from this organisation?
            Their tickets and messages remain.
          </p>
          <DialogFooter>
            <Button variant="outline" onClick={() => setRemoveDialog({ open: false, orgId: '', member: null })}>Cancel</Button>
