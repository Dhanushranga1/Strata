...
import {
  ...
  CheckCircle2, AlertCircle, BrainCircuit, Rocket, Users,
} from 'lucide-react'
...
function StrengthBar({ strength }: { strength: number }) {
  const color = strength >= 4 ? 'bg-emerald-500' : strength >= 2 ? 'bg-amber-500' : 'bg-red-500'
  const label = strength >= 4 ? 'Strong' : strength >= 2 ? 'Medium' : 'Weak'
  const labelColor = strength >= 4 ? 'text-emerald-400' : strength >= 2 ? 'text-amber-400' : 'text-red-400'
  return (
...
function SignupPageInner() {
  ...
  const signUp = async () => {
    ...
    setDoneMessage('Check your email for a confirmation link to activate your account.')
    setDone(true)
  }
    ...
    return (
      <main className="min-h-screen bg-zinc-950 flex items-center justify-center px-6">
        ...
        >
          ...
          <h1 className="text-xl font-bold text-white font-geist">Almost there!</h1>
  ...
  return (
    <main className="min-h-screen bg-zinc-950 flex">
        ...
        <div className="space-y-8">
          <div>
            <h2 className="text-3xl font-bold text-white font-geist leading-snug mb-3">
              From sign-up to<br />live in minutes.
            </h2>
            <p className="text-zinc-400 text-sm leading-relaxed">
              Create your account, upload a doc, invite your team — CASPER handles everything
              else automatically from day one.
            </p>
          </div>
          ...
          <div className="space-y-3">
            {steps.map((s) => {
              const Icon = s.icon
              return (
                <div key={s.title} className={`flex items-start gap-3 rounded-xl border p-3.5 ${s.bg}`}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
        ...
        >
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-white font-geist mb-1.5">Create your account</h1>
            <p className="text-sm text-zinc-400">Free to start. No credit card required.</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {password && <div className="mt-2"><StrengthBar strength={strength} /></div>}
            </div>
            ...
            <div>
              ...
              {confirm && (
                <div className="flex items-center gap-1.5 mt-1.5">
                  {password === confirm
                    ? <><CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /><span className="text-xs text-emerald-400">Passwords match</span></>
                    : <><AlertCircle className="w-3.5 h-3.5 text-red-400" /><span className="text-xs text-red-400">Passwords don&apos;t match</span></>
                  }
                </div>
              )}
            </div>
