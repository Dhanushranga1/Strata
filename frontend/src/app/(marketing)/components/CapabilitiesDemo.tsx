  ...
  return (
    <div className="space-y-4 font-mono text-sm">
      <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
        ...
        <span className="text-xs bg-orange-900/40 text-orange-300 border border-orange-800/50 rounded px-2 py-0.5 font-sans">
          P2 · HIGH
        </span>
      </div>
      ...
      <div className="space-y-2.5 text-xs">
        ...
        <BarRow label="Urgency" value={0.5} color="bg-yellow-400" />
        ...
        <Row
          label="Assigned to"
          value="sarah@company.com (load: 3)"
      ...
      <div className="rounded-lg bg-zinc-900 border border-zinc-800 p-3 text-xs text-zinc-500 leading-relaxed">
        ...
        <span className="text-zinc-400">
          &ldquo;troubleshooting query; high complexity (0.78)&rdquo;
        </span>
      </div>
...
function ConfidencePanel() {
  return (
    <div className="space-y-4 font-mono text-sm">
      <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
        <span className="text-zinc-400 text-xs uppercase tracking-widest">
          CASPER Breakdown — intent: troubleshooting
        </span>
      </div>
  ...
  return (
    <div className="space-y-4 font-mono text-sm">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="h-1 mt-1 rounded-full bg-zinc-800 overflow-hidden">
                <motion.div
                  ...
                  initial={{ width: 0 }}
                  whileInView={{ width: `${((c.score - 0.6) / 0.35) * 100}%` }}
                  viewport={{ once: true }}
                  transition={{
                    duration: 0.6,
                    delay: i * 0.07,
      ...
      <div className="rounded-lg bg-zinc-900 border border-zinc-800 p-3 text-xs space-y-1">
        ...
        />
        <Row
          label="source_diversity"
          value="0.712"
          valueClass="text-blue-400"
        />
        <Row
          label="score_gap (top−2nd)"
          value="0.044"
...
function BarRow({
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="text-zinc-400 w-10 text-right shrink-0">
        {value.toFixed(2)}
      </span>
    </div>
  );
...
function Badge({
  ...
  return (
    <span
      className={`inline-block px-2 py-0.5 rounded text-[11px] border font-sans ${badgeColors[color]}`}
    >
      {children}
    </span>
  );
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-16 lg:py-24">
      <div className="text-center mb-12">
        ...
        >
          Real output from a single support ticket passing through the pipeline
          — routing decision, confidence breakdown, and retrieval trace.
        </motion.p>
      </div>
