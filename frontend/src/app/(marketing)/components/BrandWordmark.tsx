...
export default function BrandWordmark({
  ...
  return (
    ...
    >
      ...
      {variant === "aurora" && <Aurora sizeClasses={sizeClasses[size]} showTM={showTM} />}
      {variant === "neon" && <Neon sizeClasses={sizeClasses[size]} showTM={showTM} />}
      {variant === "mono" && <Mono sizeClasses={sizeClasses[size]} showTM={showTM} />}
...
function Aurora({ sizeClasses, showTM }: { sizeClasses: string; showTM: boolean }) {
  return (
    <div className="relative">
      ...
      >
        TicketPilot{showTM && <sup className="ml-1 align-super text-xs text-white/70">™</sup>}
        {/* gentle light sweep */}
        <span
          aria-hidden
          className="pointer-events-none absolute inset-0 opacity-80"
          style={{
            background: "linear-gradient(0deg,rgba(255,255,255,.5),rgba(255,255,255,.08))",
            WebkitMaskImage:
...
function Neon({ sizeClasses, showTM }: { sizeClasses: string; showTM: boolean }) {
  return (
    <span className={clsx("relative inline-block font-extrabold leading-none", sizeClasses)}>
      {/* glow layer */}
      <span
        aria-hidden
      ...
      >
        TicketPilot{showTM && <sup className="ml-1 align-super text-xs text-white/80">™</sup>}
      </span>
    </span>
  );
}
...
function Mono({ sizeClasses, showTM }: { sizeClasses: string; showTM: boolean }) {
  return (
    <span className={clsx("inline-flex items-baseline gap-1.5 leading-none", sizeClasses)}>
      <span className="font-semibold text-white/90">Ticket</span>
      <span
        className="font-extrabold bg-clip-text text-transparent"
        style={{
          backgroundImage: "linear-gradient(90deg,#60a5fa 0%,#a78bfa 60%,#f472b6 100%)",
        }}
      >
        Pilot
      </span>
